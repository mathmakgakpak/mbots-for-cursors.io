...
cheatDiv.innerHTML += `<div><button onclick="movement = !movement">movement</button></div>`
//cheatDiv.innerHTML += `<div><button onclick="manualcontrol = !manualcontrol">manual control dont use cuz ded</button></div>`
//cheatDiv.innerHTML += `<div style="width: 20%; height: 30%; text-align: center" id="controls"> <button onclick="ws.send(JSON.stringify({'eval': 'up', 'x': serverx, 'y': servery}))">↑</button><br> <button onclick="ws.send(JSON.stringify({'eval': 'left', 'x': serverx, 'y': servery}))">←</button><button></button><button onclick="ws.send(JSON.stringify({'eval': 'right', 'x': serverx, 'y': servery}))">→</button><br> <button onclick="ws.send(JSON.stringify({'eval': 'down', 'x': serverx, 'y': servery}))">↓</button></div>`
cheatDiv.innerHTML += "<br>";
...
setTimeout(function() {
	document.getElementsByTagName("div")[3].appendChild(cheatDiv)
}, 2000)
var moveInterval;
var savedx;
var savedy;
...
function drawImage(img, x = savedx, y = savedy) {
	switch (img.toLowerCase()) {
		case "arrowleft":
			drawArray(imagesToDraw.arrowLeft, serverx, servery);
			break;
		case "arrowup":
			drawArray(imagesToDraw.arrowUp, serverx, servery);
			break;
		case "arrowdown":
			drawArray(imagesToDraw.arrowDown, serverx, servery);
			break;
		case "arrowright":
			drawArray(imagesToDraw.arrowRight, serverx, servery);
			break;
		case "cursorinrect":
			drawArray(imagesToDraw.cursorInRect, x, y);
			break;
		case "cursor":
			drawArray(imagesToDraw.cursor, x, y);
			break;
		case "cutesmile":
			drawingScale *= 2
			drawArray(imagesToDraw.cuteSmile, x, y);
			drawingScale /= 2
			break;
